...
| 前端                                 | done     | 问题全部解决          |
| rtsp推流卡顿，画面不稳定，画质不清晰 | todo     | 下周中旬开始排查        |
|                                   |          |                    |
| 网络OTA升级                         | todo     | 做后台服务，检测标志位更新，删掉替换原来app，关键在于http协议交互 |
| 网络模型下发                         | todo     | 关键在于http协议交互，下载模型 |
| 网络音柱开发                         | todo     |                      |
| 网络告警通知                         | doing    | 关键在于http协议交互，上传图片和告警类型 |
| 转码卡支持rtmp推流（简单）                | doing | 1.先完成两路保存 2.再完成推流 |
| 算法检测导致系统有问题 | doing |  |
